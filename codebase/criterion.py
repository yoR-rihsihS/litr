...
class SetCriterion(nn.Module):
    """ This class computes the loss for LiTr
    ...
    def __init__(self, matcher, weight_dict, losses, num_classes, share_matched_indices=False, alpha=0.25, gamma=2.0):
        ...
        Args:
            - num_classes (dict): Contains as key the label name and as values their number of possible characters, omitting the special no-object category
    ...
    def loss_labels_focal(self, outputs, targets, indices, num_boxes, name="label_0"):
        """
        ...
        """
        assert f"pred_{name}" in outputs, f"Model output does not contain pred_{name}"
        idx = self._get_src_permutation_idx(indices)
        src_logits = outputs[f"pred_{name}"]

        target_classes_o = torch.cat([t[name][J] for t, (_, J) in zip(targets, indices)])
        target_classes = torch.full(src_logits.shape[:2], self.num_classes[name], dtype=torch.int64, device=src_logits.device)
        target_classes[idx] = target_classes_o

        target = F.one_hot(target_classes, num_classes=self.num_classes[name]+1)[..., :-1].float()
        loss = ops.sigmoid_focal_loss(src_logits, target, self.alpha, self.gamma, reduction='none')
        loss = loss.mean(1).sum() * src_logits.shape[1] / num_boxes

        return {f"loss_{name}": loss}

    def loss_boxes(self, outputs, targets, indices, num_boxes, name="boxes"):
        """
        ...
        assert f"pred_{name}" in outputs, f"Model output does not contain pred_{name}"
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs[f"pred_{name}"][idx]

        target_boxes = torch.cat([t[name][J] for t, (_, J) in zip(targets, indices)], dim=0)
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes):
        function_map = {
            ...
            "label_0" : self.loss_labels_focal,
            "label_1" : self.loss_labels_focal,
            "label_2" : self.loss_labels_focal,
            "label_3" : self.loss_labels_focal,
            "label_4" : self.loss_labels_focal,
            "label_5" : self.loss_labels_focal,
            "label_6" : self.loss_labels_focal,
        }
        assert loss in function_map, f"do you really want to compute {loss} loss?"
        return function_map[loss](outputs, targets, indices, num_boxes, loss)
